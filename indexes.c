...
void SearchIndexes( EntryList* hamming_results,
  ...
  for(i=0;i!=docsize;i++){  //for every word search all indexes
    ...
    found_in_cache = Search_Cache(&cache[c],*doc.words[i],edit_results,&bucket);
    ...
    if(!found_in_cache) //word was not found in cache
      SearchEditIndex(indexes.edit,*doc.words[i],
                          edit_results,edit_results_cache);

    SearchExactIndex(indexes.exact,*doc.words[i],exact_results);

    SearchHammingIndex(indexes.hamming,*doc.words[i],hamming_results);

    Update_Cache(&cache[c],*doc.words[i],edit_results_cache,
                                                    bucket,found_in_cache); 
...
void SearchEditIndex(Edit_Index index,
  ...
  for(i=0;i!=4;i++)
    SearchVpTree(&word,i,index.vp_tree.root,
                &result[i],&result_cache[i],MT_EDIT_DIST);
...
void BuildHammingIndex(Hamming_Index* index,
  ...
  for(i = 0; i != table_size; i++){ 
    if(ElistTable[i].size)
      BuildVpTree(index->vp_table[i].root,&ElistTable[i],MT_HAMMING_DIST);
...
void BuildExactIndex(Exact_Index* index, EntryList* entries){
  ...
  for(i = 0; i != esize ; i++){
    ...
    index->HashTable[bucket] = InsertEntryInBTree(index->HashTable[bucket],
                                            entries->elist[i]);
  }
