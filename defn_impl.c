...
void PrintQuery(Query* query){
  ...
  if(query->match_type == MT_EXACT_MATCH) printf("EXACT MATCH\n");
  if(query->match_type == MT_HAMMING_DIST) printf("HAMMING_MATCH\n");
  if(query->match_type == MT_EDIT_DIST) printf("EDIT_MATCH\n");
...
void DeDuplicateDocWord(BT_Node** HashTable,Word cword,
                        int* unique_words,Document* document,int bucket){
...
int EditDistance(const Word* w1, const Word* w2) {
  ...
  for(i=1;i <= w1->length;i++){   //memory optimized Needleman-Wunsch algorithm
    ...
    for(j=1;j <= w2->length;j++){
      ...
      dist = d1;
      if(d2 < dist) dist = d2;  //we want the minimum
      if(d3 < dist) dist = d3;
