...
enum ErrorCode CreateEntry(const Word* word, Entry* entry) {
  ...
  entry->payload.max_size = 512;
  entry->payload.table = malloc(entry->payload.max_size*sizeof(Payload_Member));
...
void UpdatePayload(Entry* entry,int query_id,int position) {
    ...
    if(size == entry->payload.max_size){
...
void CopyPayload(Entry* dest,const Entry* source){
  ...
  if(dest->payload.max_size != payloadmaxsize ){  
    dest->payload.table =         
                    realloc(dest->payload.table,  
