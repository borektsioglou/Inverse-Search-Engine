...
int Find_Vp_Median(EntryList* vp_candidates,
...

{
  ...
  for(i = 0;i != total_vp_candidates;i++){
    ...
    standard_dev[i] = standard_deviation(vp_cand_edit[i] ,total_vp_candidates);
  ...
  if(!subset_size%2)  //take median
    *median = (vp_cand_edit[max][subset_size/2] + 
                                vp_cand_edit[max][(subset_size/2)+1])/2 ;     
  else{
    *median = vp_cand_edit[max][subset_size/2];
  }
  return max;
...
void GenerateNewSets(EntryList* left_subtree,
                    ...
                    enum MatchType type)
{
  ...
  for(i = 0;i != total_entries;i++){
    if(i!=vp){
      if(type == MT_EDIT_DIST)
        dist = EditDistance(entries->elist[vp].word,entries->elist[i].word);
      else
        dist = HammingDistance(entries->elist[vp].word,entries->elist[i].word);
                ...
                enum MatchType type)
{
  ...
  GenerateNewSets(&left_subtree,&right_subtree,entries,node,vp,median,type);
...
void SearchVpTree(const Word* word,
                  int threshold,
                  const VPTnode* node,
                  EntryList* result,
                  ...
                  enum MatchType type)
{
  ...
  if(node->dataset!=NULL){  //this is leaf node with some entries in dataset
    int tempdist,i;
