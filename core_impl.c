...
void ThreadMatchDocument(match_data* args){
  ...
  SearchIndexes(hamming_results,edit_results,&exact_results,doc,indexes,c_index);
  //----------------------------------------------------------------------------
  ...
  id_results_size = SearchQueriesBitMap(&results_s,hamming_results,
                                  edit_results,exact_results,queries.general);
...
enum ErrorCode GetNextAvailRes(DocID*         p_doc_id,
  ...
  if(!globalfs){  //if this is the first GetNextAvailRes
    ThPool_Wait(threadpool);  //wait all mathcdocs to terminate
    globalfs = 1;           
  }
